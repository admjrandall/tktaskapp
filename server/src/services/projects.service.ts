...
export class ProjectsService {
  ...
  async getById(tenantId: string, id: string): Promise<Project | null> {
    return withTenant(tenantId, async (tx) => {
      const rows = await tx
        ...
        .from(projects)
        .where(
          and(eq(projects.id, id), eq(projects.tenantId, tenantId), isNull(projects.deletedAt)),
        )
        .limit(1)
      return rows[0] ?? null
    })
  ...
  async create(tenantId: string, userId: string, data: CreateProjectInput): Promise<Project> {
    return withTenant(tenantId, async (tx) => {
      const [row] = await tx
        .insert(projects)
