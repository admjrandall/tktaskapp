...
authRouter.get(
  ...
  describeRoute({
    tags: ['Auth'],
    summary: 'Initiate OIDC PKCE login',
    responses: {
...
authRouter.get(
  ...
  describeRoute({
    ...
    responses: { 302: { description: 'Redirect to app' }, 401: { description: 'Auth failure' } },
  }),
  async (c) => {
    const config = _oidcConfig()
...
authRouter.post(
  ...
  describeRoute({
    ...
    responses: {
      ...
      401: { description: 'No or replayed refresh token' },
    },
  }),
  async (c) => {
    const config = _oidcConfig()
...
authRouter.post(
  ...
  describeRoute({
    tags: ['Auth'],
    summary: 'Clear refresh cookie and revoke tokens',
...
authRouter.post(
  ...
  describeRoute({
    tags: ['Auth'],
    summary: 'Step-up auth (deprecated — use /step-up/start)',
    responses: { 400: { description: 'Use nonce-bound flow instead' } },
  }),
  authMiddleware as MiddlewareHandler,
  async (c) => {
...
authRouter.post(
  ...
  describeRoute({
    tags: ['Auth'],
    summary: 'Start nonce-bound step-up re-authentication',
    responses: {
      200: { description: 'Authorization URL and TTL' },
      400: { description: 'Invalid origin or operation' },
    },
  }),
  authMiddleware as MiddlewareHandler,
  async (c) => {
