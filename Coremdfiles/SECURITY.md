...
1. **Constrained Markdown renderer** (`_mdToHtml` in `documents.ts`) — processes input line-by-line. Every text segment is HTML-escaped (`&`, `<`, `>`, `"` → entities) _before_ being wrapped in a tag. Only a small set of structural tags can be emitted (`h1`–`h3`, `p`, `ul`, `ol`, `li`, `pre`, `code`, `blockquote`, `table`, `hr`). Raw HTML in model output becomes visible escaped text, not markup. Model instructions request Markdown output; the renderer enforces it regardless.
...
**Email:** [developer@techkeycloud.com](mailto:developer@techkeycloud.com)
...
**PGP encryption** — not currently required. If you need to share sensitive details, request a PGP key before sending.
...
| Fix or documented mitigation   | Varies by severity: Critical ≤ 7 days, High ≤ 30 days, Medium ≤ 90 days |
| Public disclosure              | After fix is released and users have had reasonable time to update      |

### Do-not-disclose policy

- **Do not** open a public GitHub issue or discussion for security vulnerabilities.
- **Do not** disclose publicly before a fix has been released and coordinated.
- **Do not** use automated scanners to test against production systems you do not own.

We follow [responsible disclosure](https://en.wikipedia.org/wiki/Responsible_disclosure). If you follow these guidelines, we commit to not taking legal action against you and to crediting you in the release notes (unless you prefer anonymity).

### Out of scope

The following are accepted risks and will not be treated as vulnerabilities:

- An attacker with active code execution on the device (OS compromise — out of scope for a browser app)
- Browser engine vulnerabilities (report to the browser vendor)
- Vulnerabilities in test/development environments that are not reachable in production
- Self-XSS (requires the attacker to have already compromised the browser session)
- Social engineering attacks
- DoS attacks that require physical access to the device
- Disclosure of non-sensitive preferences (theme, dashboard layout, AI tier selection)
- Attacks that require the vault password to be known

### Bug bounty

There is no formal bug bounty programme at this time. Significant findings will be credited in the release notes.
