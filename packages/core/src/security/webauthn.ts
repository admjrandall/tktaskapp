...
export async function registerPasskey(masterPassword: string): Promise<PasskeyCredential> {
  ...
  const prfResult = (cred.getClientExtensionResults() as Record<string, unknown>)['prf'] as
    | PRFResult
    | undefined
  const prfOutput = prfResult?.results?.first

  if (!prfOutput) {
    throw new Error('PRF extension not supported by this authenticator. Use TOTP instead.')
  }
...
export async function authenticateWithPasskey(cred: PasskeyCredential): Promise<string> {
  ...
  const assertion = (await navigator.credentials.get({
    publicKey: {
      ...
      userVerification: 'required',
      extensions: {
        prf: { eval: { first: prfSalt.buffer } } as unknown as AuthenticationExtensionsClientInputs,
      } as AuthenticationExtensionsClientInputs,
    },
  })) as PublicKeyCredential

  type PRFResult = { results?: { first?: ArrayBuffer } }
  const prfResult = (assertion.getClientExtensionResults() as Record<string, unknown>)['prf'] as
    | PRFResult
    | undefined
  const prfOutput = prfResult?.results?.first
...
export async function loadPasskeys(): Promise<PasskeyCredential[]> {
  ...
  try {
    ...
    const rec = records.find((r: Record<string, unknown>) => r['id'] === '__mfa_passkeys__') as
      | PasskeyStore
      | undefined
