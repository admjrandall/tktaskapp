...
import { validateImportPayload } from '../schemas/import.schema.js'
...
export function _vaultDbOpen(): Promise<IDBDatabase> {
  return new Promise((res, rej) => {
    ...
    req.onupgradeneeded = (e) =>
      (e.target as IDBOpenDBRequest).result.createObjectStore(VAULT_META_STORE)
    req.onsuccess = (e) => {
...
export async function _vaultMetaGet(k: string): Promise<unknown> {
  try {
    ...
    return await new Promise<unknown>((res, rej) => {
      ...
      req.onerror = () => {
        rej(req.error ?? new Error('IDB get failed'))
      }
    })
  } catch {
    return null
...
export async function _vaultMetaSet(k: string, v: unknown): Promise<void> {
  try {
    ...
    await new Promise<void>((res, rej) => {
      ...
      tx.onerror = () => {
        rej(tx.error ?? new Error('IDB put failed'))
      }
    })
  } catch (e) {
...
export async function loadVault(key: CryptoKey): Promise<Record<string, unknown[]>> {
  ...
  try {
    ...
    throw new Error(
      'Vault decryption failed. Your data may be corrupted. Do not save until resolved.',
    )
  }
}
...
interface _EncryptedBackupPayload {
  v: number
  salt: string
  data: string
  ts?: string
...
export async function importEncryptedBackup(
  ...
  const p = JSON.parse(json) as _EncryptedBackupPayload
  ...
  if (legacyKdfUsed) {
    console.warn(
      '[vault] importEncryptedBackup: backup used legacy 310k-iteration KDF. Consider exporting a fresh backup after this import.',
    )
  }
...
export async function importJSON(json: string, key: CryptoKey): Promise<unknown> {
  ...
  const validation = validateImportPayload(v)
  ...
  for (const [k, records] of Object.entries(validation.data)) {
  ...
  if (skippedIdb.length > 0) {
    console.warn(
      `[vault] importJSON: skipped IDB-backed stores (${skippedIdb.join(', ')}) — per-record encryption prevents bulk vault import.`,
    )
  }
