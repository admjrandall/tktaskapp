...
export async function _idbLoadStore(storeName: string, cryptoKey: CryptoKey): Promise<unknown[]> {
  try {
    ...
    const records = await new Promise<Array<{ id: string; enc: string }>>((res, rej) => {
      ...
      req.onsuccess = () => res((req.result as Array<{ id: string; enc: string }>) || []);
      req.onerror = () => rej(req.error);
    });
    const decrypted = await Promise.all(records.map(async r => {
      try { return await aesDecrypt(cryptoKey, r.enc); }
      catch { console.warn(`[idb] Failed to decrypt ${storeName}/${r.id}`); return null; }
    }));
