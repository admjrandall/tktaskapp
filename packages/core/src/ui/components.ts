...
export function renderToast(toast: Toast): string {
  ...
  return `<div class="toast-container"><div class="toast toast-${toast.type}" role="alert">${icon} <span>${escH(toast.message)}</span></div></div>`
}
...
export function renderAvatar(name: string | null | undefined, size = 'md', style = ''): string {
  ...
  return `<div class="avatar avatar-${size}" style="background:${bg};color:${fg};${style}">${initials(name)}</div>`
}
export function renderPriorityBadge(p: string | null | undefined): string {
  ...
  return p ? `<span class="badge ${m[p] || 'badge-slate'}">${p}</span>` : ''
}
export function renderDueBadge(ds: string | null | undefined): string {
  if (!ds) return ''
  const d = daysUntil(ds)
  if (d === null) return ''
  if (d < 0) return `<span class="badge badge-rose">${Math.abs(d)}d overdue</span>`
  if (d === 0) return `<span class="badge badge-amber">Due today</span>`
  if (d <= 3) return `<span class="badge badge-amber">Due in ${d}d</span>`
  return `<span class="badge badge-slate">${formatDate(ds)}</span>`
}
export function renderSpinner(msg = ''): string {
  return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.75rem;padding:3rem;color:var(--text-tertiary)"><div class="spinner"></div>${msg ? `<p style="font-size:.875rem">${msg}</p>` : ''}</div>`
}
...
export function renderSearchInput(val: unknown = '', ph = 'Search…', id = 'search-input'): string {
  return `<div style="position:relative;display:flex;align-items:center"><span style="position:absolute;left:.625rem;color:var(--text-tertiary);pointer-events:none">${Icons.Search(14)}</span><input class="input" id="${id}" value="${escH(val)}" placeholder="${ph}" style="padding-left:2rem;width:220px;height:34px" autocomplete="off"></div>`
}
  ...
  },
  {
    ...
    action: () => {
      if (aiNeedsOnboarding()) {
        ...
        setState({ aiPanelOpen: !getState().aiPanelOpen })
      }
    },
  },
  {
