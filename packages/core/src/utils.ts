...
export function sortRecords<T extends AnyRec>(recs: T[], field: string, dir: 'asc' | 'desc' = 'asc'): T[] {
...
export function filterRecords<T extends AnyRec>(recs: T[], filters: Record<string, unknown>): T[] {
  return recs.filter((r: AnyRec) => {
    for (const [k, v] of Object.entries(filters)) {
      ...
      } else if (String(rv || '').toLowerCase() !== String(v).toLowerCase()) {
        return false;
      }
    }
    return true;
...
export function searchRecords<T extends AnyRec>(recs: T[], q: string, fields: string[]): T[] {
  ...
  return recs.filter((r: AnyRec) =>
    fields.some(f => String(r[f] || '').toLowerCase().includes(lq))
  );
}

export function debounce<F extends (...a: unknown[]) => unknown>(fn: F, ms: number): (...a: Parameters<F>) => void {
...
export function toCSV(recs: AnyRec[], fields: string[]): string {
  ...
  return [
    fields.join(','),
    ...recs.map(r => fields.map(f => `"${String(r[f] ?? '').replace(/"/g, '""')}"`).join(',')),
  ].join('\n');
}
