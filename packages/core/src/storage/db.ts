...
import { loadVault, saveVault } from '../security/vault.js'
import { _idbLoadStore, _idbPutRecord, _idbDeleteRecord, _idbClearStore } from './idb-data.js'
...
export async function dbInit(cryptoKey: CryptoKey): Promise<Record<string, unknown[]>> {
  ...
  _dbData.documents = (await _idbLoadStore('documents', _dbKey)).filter(
    (r) => !INTERNAL_DOCUMENT_IDS.has((r as { id?: string } | null)?.id ?? ''),
  )
...
export async function dbFlush(): Promise<void> {
  ...
  try {
    ...
    /* fs module not available — silently skip */
  }
}
...
export function _scheduleFlush(): void {
  ...
  _flushTimer = setTimeout(() => {
    _flushTimer = null
    dbFlush().catch((e: unknown) => {
      console.error('[db] flush failed:', (e as Error).message)
    })
  }, 300)
}
...
export function dbGetAll(s: string): unknown[] {
  ...
  if (s === 'documents')
    return rows.filter((r) => !INTERNAL_DOCUMENT_IDS.has((r as { id?: string } | null)?.id ?? ''))
  return [...rows]
}
export function dbGetById(s: string, id: string): unknown {
...
export async function dbUpdate(
  ...
  const up = { ...arr[idx], ...changes, updatedAt: nowISO() } as {
    id: string
    [k: string]: unknown
  }
  arr[idx] = up
...
export async function checkDueDates(): Promise<void> {
  ...
  const tasks = (getStore('tasks') as Task[]).filter(
  ...
  for (const t of tasks) {
    ...
    const nid = 'due_' + t.id
    if (
      (getStore('notifications') as Array<{ relatedId?: string }>).find((n) => n.relatedId === nid)
    )
      continue
    const diff = Math.round((due.getTime() - today.getTime()) / 86400000)
    if (diff < 0)
      await createNotification('Task Overdue', `"${t.title}" was due ${t.dueDate}`, 'error', nid)
    else if (diff === 0)
      await createNotification('Due Today', `"${t.title}" is due today`, 'warning', nid)
    else if (diff <= 3)
      await createNotification(
        'Due Soon',
        `"${t.title}" is due in ${diff} day${diff === 1 ? '' : 's'}`,
        'info',
        nid,
      )
  }
...
export async function stopTimer(id: string): Promise<unknown> {
...
export function getRunningTimer(): unknown {
  return (getStore('timeEntries') as Array<{ running: boolean }>).find((e) => e.running) || null
}

export function globalSearch(
  q: string,
): { store: string; id: string; label: string; icon: string }[] {
...
export function _getDbKey(): CryptoKey | null {
  return _dbKey
}
export function _getDbData(): Record<string, unknown[]> {
  return _dbData
}
export function _setDbData(d: Record<string, unknown[]>): void {
  _dbData = d
}
