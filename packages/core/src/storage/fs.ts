...
import { _vaultMetaGet, _vaultMetaSet } from '../security/vault.js'
...
export function isFsReady(): boolean {
  return _fsReady && !!_fsHandle
}
export function getFsHandle(): FileSystemFileHandle | null {
  return _fsHandle
}
export function getFsLastSave(): string | null {
  return _fsLastSave
}
...
async function _idbSet(key: string, val: unknown): Promise<void> {
  ...
  return new Promise((res, rej) => {
    ...
    tx.onerror = () => {
      rej(tx.error ?? new Error('IDB set failed'))
    }
  })
}
...
export async function fsUnlink(): Promise<void> {
  ...
  return new Promise((res, rej) => {
    ...
    tx.onerror = () => {
      rej(tx.error ?? new Error('IDB delete failed'))
    }
  })
}
