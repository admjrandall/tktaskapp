...
export type AuditEventType =
  | 'session_start'
  | 'auth_success' | 'auth_failure' | 'auth_locked' | 'auth_unlocked'
  | 'mfa_success' | 'mfa_failure' | 'mfa_enabled' | 'mfa_disabled'
  | 'passkey_registered' | 'passkey_removed'
  | 'app_locked' | 'app_unlocked'
  | 'password_changed'
  | 'vault_file_opened' | 'vault_exported' | 'vault_imported'
  | 'backup_exported' | 'backup_imported'
  | 'data_exported' | 'data_imported'
  | 'app_reset'
  | 'ai_key_added' | 'ai_key_removed'
  | 'ai_query';
...
async function _loadRaw(): Promise<AuditEntry[]> {
  ...
  try {
    ...
    const logRec = records.find((r: Record<string, unknown>) => r['id'] === '__audit_log__') as Record<string, unknown> | undefined;
    if (!logRec) return [];
...
function _applyAutoPurge(entries: AuditEntry[]): AuditEntry[] {
  const daysStr = typeof localStorage !== 'undefined'
    ? localStorage.getItem('taskapp_audit_purge_days')
    : null;
