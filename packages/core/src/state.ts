...
import { dbGetAll, getRunningTimer } from './db.js';
...
let _state: AppState = {
  authed: false, cryptoKey: null, currentView: 'dashboard', sidebarCollapsed: false,
  ...
  aiPanelOpen: false, commandOpen: false, notifPanelOpen: false,
  toast: null, confirmDialog: null, recordModal: null, docModal: false, fileViewer: null,
  runningTimer: null, timerElapsed: 0,
};
...
function _notify(): void { _listeners.forEach(fn => fn(_state)); }
...
export function reloadData(): void {
  ...
  stores.forEach(s => { (patch as Record<string, unknown>)[s] = dbGetAll(s); });
  const running = (patch.timeEntries as Array<{ running: boolean }> | undefined)?.find(e => e.running) ?? getRunningTimer();
...
export function closeConfirm(): void { setState({ confirmDialog: null }); }
...
export function closeRecordModal(): void { setState({ recordModal: null }); }
...
export function navigate(view: string): void {
  if (view === 'ai' && _aiNeedsOnboarding && _aiNeedsOnboarding()) {
    if (_openAIWizard) { _openAIWizard(1); return; }
  }
  setState({
    currentView: view, aiPanelOpen: false, commandOpen: false, notifPanelOpen: false,
  });
}
