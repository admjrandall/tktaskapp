...
import {
  aiRuntime, STATIC_MODELS,
  modelChipLabel, backendSubtitle, backendLabel,
  activeModelId, preferredModelId,
  fetchOllamaModels, selectModel,
  startAILoad, callBackend,
} from './ai-runtime.js';
...
export function setAIUISchemas(schemas: Record<string, { fields: Array<{ key: string; type: string; store?: string }> }>): void {
  setAIToolsHooks({ SCHEMAS: schemas, streamToBubble, finalRender: (ctx) => finalRender(ctx) });
}
...
export function renderAIStatus(): string {
  if (aiRuntime.ready) return `<span class="ai-status"><span class="ai-pulse"></span> ${backendLabel()}</span>`;
  if (aiRuntime.loadStarted) {
    const extra = aiRuntime.backend === 'webllm' && aiRuntime.webllmLoadProgress > 0
...
export function renderAIPanel(open: boolean): string {
  if (!open || (getState() as AppState).currentView === 'ai') return '';
...
export function renderChatBody(compact: boolean): string {
  ...
  const msgs = aiRuntime.history.filter(m => m.role !== 'system').map(m => {
  ...
  const suggests = aiRuntime.ready && aiRuntime.history.length === 0 && !aiRuntime.pendingAction ? `
    <div style="padding:1rem;display:flex;flex-direction:column;gap:.5rem">
      ...
      ${["Create a task named Task 1 due Friday", "How many overdue tasks?", "Show all active clients", "Start a timer on Task 1", "Open the Projects view"]
          .map(s => `<button class="btn btn-secondary btn-sm" style="text-align:left;justify-content:flex-start" data-suggest="${escH(s)}">${escH(s)}</button>`).join('')}
    </div>` : '';
...
function renderModelDropdown(ns: string): string {
  ...
  const sections = [
    ...(isAITierAllowed('browser') ? [
      ...
      { title: 'In-browser (WebGPU)',  items: STATIC_MODELS.filter(m => m.backend === 'webllm') },
    ] : []),
    ...(isAITierAllowed('ollama') ? [
      { title: ollamaList.length ? `Local/Internal AI (${ollamaList.length})` : 'Local/Internal AI (not detected)', items: ollamaList as typeof STATIC_MODELS },
    ] : []),
  ];
  const items = sections.map(sec => {
    ...
    return `<div style="padding:.5rem .75rem .25rem;font-size:.7rem;text-transform:uppercase;letter-spacing:.05em;color:var(--text-tertiary);font-weight:600">${escH(sec.title)}</div>` +
      sec.items.map(m => {
...
export function bindAIPanel(): void {
  document.getElementById('ai-panel-close')?.addEventListener('click', () => setState({ aiPanelOpen: false }));
  document.getElementById('ai-expand-btn')?.addEventListener('click', () => setState({ aiPanelOpen: false, currentView: 'ai' }));
  ...
  document.getElementById('ai-settings-btn-panel')?.addEventListener('click', () => {
    _setSettingsSection('ai'); setState({ aiPanelOpen: false, currentView: 'settings' });
  });
...
export function bindAIChatWorkspace(): void {
  ...
  document.getElementById('ai-settings-btn')?.addEventListener('click', () => { _setSettingsSection('ai'); navigate('settings'); });
  document.getElementById('ai-open-wizard-empty')?.addEventListener('click', () => _openAIWizard(1));
  document.getElementById('ai-open-wizard-toolbar')?.addEventListener('click', () => _openAIWizard(1));
...
function bindChatInput(ctx: string): void {
  ...
  input?.addEventListener('keydown', e => {
    if ((e as KeyboardEvent).key === 'Enter' && !(e as KeyboardEvent).shiftKey) { e.preventDefault(); doSend(); }
  });
...
function bindModelPicker(ctx: string): void {
  ...
  btn?.addEventListener('click', async (e) => {
    e.stopPropagation();
    if (aiRuntime.modelPickerOpen === ns) { aiRuntime.modelPickerOpen = null; rerenderChatInputArea(ctx); return; }
  ...
  document.querySelectorAll<HTMLElement>(`#model-dropdown-${ns} [data-model-id]`).forEach(opt => {
    opt.addEventListener('click', async (e) => {
      ...
      const entry = STATIC_MODELS.find(m => m.id === modelId) ||
                    (aiRuntime.ollamaModelsCache || []).find(m => m['id'] === modelId) as typeof STATIC_MODELS[0] | undefined;
      if (!entry) return;
      aiRuntime.modelPickerOpen = null;
      await selectModel(entry as typeof STATIC_MODELS[0]);
    });
  });
}
...
function rerenderChatInputArea(ctx: string): void {
  if (ctx === 'panel') {
    ...
    if (pb) {
      ...
      pb.innerHTML = renderChatBody(true);
      bindAIPanel();
...
function finalRender(ctx: string): void {
  if (ctx === 'panel') {
    const pb = document.querySelector('#ai-panel .panel-body');
    if (pb) { pb.innerHTML = renderChatBody(true); bindAIPanel(); }
  } else {
    _appRenderWorkspace('ai');
  }
}
  ...
  if (ctx === 'panel') {
    ...
    if (msgs && !aiRuntime.streaming) {
      ...
      if (userMsg?.role === 'user') {
        msgs.innerHTML += `<div class="chat-message user"><div class="chat-bubble">${escH(userMsg.content)}</div></div>
...
function scrollChatBottom(): void {
  setTimeout(() => {
    document.querySelectorAll('[id^="chat-messages"]').forEach(el => { el.scrollTop = el.scrollHeight; });
  }, 50);
}
...
export async function sendAIMessage(text: string, ctx: string): Promise<void> {
  ...
  try {
    if (!aiRuntime.ready) {
      aiRuntime.history.push({ role: 'assistant', content: 'AI not connected yet. Click "Connect AI" to get started.' });
    } else {
      aiRuntime.abortController = new AbortController();
      const raw = await callBackend(buildSystemPrompt(), aiRuntime.history, streamToBubble, aiRuntime.abortController.signal);
