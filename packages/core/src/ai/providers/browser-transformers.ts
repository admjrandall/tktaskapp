...
export async function loadWebLLM(
  ...
  if (!('gpu' in navigator)) throw new Error('WebGPU not supported. Use Chrome/Edge 113+ on desktop.');
  ...
  const processor = await ((AutoProcessor as unknown as AnyRecord)['from_pretrained'] as (...a: unknown[]) => Promise<AnyRecord>)(hfModelId, { progress_callback });
  const model     = await ((Gemma4ForConditionalGeneration as unknown as AnyRecord)['from_pretrained'] as (...a: unknown[]) => Promise<AnyRecord>)(hfModelId, {
  ...
  const warmIn = await (processor as unknown as (...a: unknown[]) => Promise<AnyRecord>)(warmMsg, null, null, { add_special_tokens: false });
  let warm = '';
  const warmStreamer = new (TextStreamer as unknown as new (...a: unknown[]) => unknown)(processor['tokenizer'], {
    skip_prompt: true, skip_special_tokens: true,
    callback_function: (t: string) => { warm += t; },
  });
  await (model['generate'] as (...a: unknown[]) => Promise<unknown>)({ ...(warmIn as AnyRecord), max_new_tokens: 8, do_sample: false, streamer: warmStreamer });
...
export async function callWebLLM(
  ...
  const proc = processor as AnyRecord;
  const mdl  = model as AnyRecord;
  ...
  const inputs = await (proc as unknown as (...a: unknown[]) => Promise<AnyRecord>)(prompt, null, null, { add_special_tokens: false });

  let full = '';
  const streamer = new TS(proc['tokenizer'], {
    skip_prompt: true, skip_special_tokens: true,
    callback_function: (text: string) => { full += text; onToken(full); },
  });
  await (mdl['generate'] as (...a: unknown[]) => Promise<unknown>)({ ...(inputs as AnyRecord), max_new_tokens: 512, do_sample: false, streamer });
  return (full || '').trim();
}
