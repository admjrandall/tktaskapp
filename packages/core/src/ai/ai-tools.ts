...
import { nowISO, dbGetById, dbCreate, dbUpdate, softDelete, getStore, getRunningTimer, startTimer, stopTimer } from '../db.js';
...
export const AI_TOOLS: Record<string, { desc: string; args: string[] }> = {
  create_record:    { desc: 'Create a new record. store: tasks|clients|projects|people|departments|communications|files. fields: object matching schema.', args: ['store', 'fields'] },
  update_record:    { desc: 'Update an existing record. id_or_name resolves by id, exact name/title, or fuzzy match. changes: object of fields to set.', args: ['store', 'id_or_name', 'changes'] },
  delete_record:    { desc: 'Soft-delete a record to Trash.', args: ['store', 'id_or_name'] },
  query_records:    { desc: 'List records. Optional filter: status, priority, dueDate, overdue (bool), dueToday (bool), dueSoon (bool), clientId/clientName, assigneeId/assigneeName, stage, search (substring).', args: ['store', 'filter?'] },
  count_records:    { desc: 'Count records matching a filter (same shape as query_records).', args: ['store', 'filter?'] },
  summarize_record: { desc: 'Return a detailed summary of one record (including related records).', args: ['store', 'id_or_name'] },
  ...
  answer_question:  { desc: 'Fallback for general questions. Executor injects a CRM summary and re-prompts the model.', args: ['question'] },
};

export function buildSystemPrompt(): string {
  const toolLines = Object.entries(AI_TOOLS).map(([name, t]) => {
...
export async function handleModelOutput(raw: string, ctx: string): Promise<void> {
  if (!raw) { aiRuntime.history.push({ role: 'assistant', content: '(no response)' }); return; }
  const tc = extractToolCall(raw);
  if (!tc) { aiRuntime.history.push({ role: 'assistant', content: raw }); return; }
  await routeToolCall(tc, ctx);
}

const READ_ONLY_TOOLS = new Set(['query_records', 'count_records', 'summarize_record', 'get_running_timer', 'list_files', 'answer_question', 'navigate']);
...
export async function routeToolCall(tc: AnyRecord, ctx: string): Promise<void> {
  ...
  if (!AI_TOOLS[tool]) {
    aiRuntime.history.push({ role: 'assistant', content: `(Unknown tool "${tool}". I'll answer in plain text instead.)` });
    return;
  }
  if (READ_ONLY_TOOLS.has(tool)) {
    try {
      ...
      aiRuntime.history.push({ role: 'assistant', content: `Couldn't run ${tool}: ${(e as Error).message}` });
    }
    return;
  }
...
export function summarizeAction(tool: string, args: AnyRecord): string {
  if (tool === 'create_record') return `Create ${String(args['store'] || 'record').replace(/s$/, '')}:\n${JSON.stringify(args['fields'] || {}, null, 2)}`;
  if (tool === 'update_record') return `Update ${args['store']} "${args['id_or_name']}":\n${JSON.stringify(args['changes'] || {}, null, 2)}`;
  if (tool === 'delete_record') return `Move to Recycle Bin: ${args['store']} "${args['id_or_name']}"`;
  if (tool === 'start_timer')   return `Start timer${args['taskId_or_name'] ? ` on "${args['taskId_or_name']}"` : ''}${args['description'] ? ` (${args['description']})` : ''}`;
  if (tool === 'stop_timer')    return 'Stop the running timer';
  if (tool === 'log_communication') return `Log ${args['type'] || 'communication'}${args['subject'] ? ` "${args['subject']}"` : ''}${args['relatedTo'] ? ` on ${(args['relatedTo'] as AnyRecord)['store']} "${(args['relatedTo'] as AnyRecord)['id_or_name']}"` : ''}:\n${args['body'] || ''}`;
  if (tool === 'attach_file')   return `Attach file "${args['name']}"${args['relatedTo'] ? ` to ${(args['relatedTo'] as AnyRecord)['store']} "${(args['relatedTo'] as AnyRecord)['id_or_name']}"` : ''}`;
...
function resolveRecord(store: string, idOrName: string): AnyRecord | null {
  ...
  return hit ?? (all.find(r => String(r[nameKey] || '').toLowerCase().includes(needle)) ?? null);
}
...
function matchesFilter(rec: AnyRecord, _store: string, filter: AnyRecord | null): boolean {
  ...
  if (filter['search']) {
    if (!JSON.stringify(rec).toLowerCase().includes(String(filter['search']).toLowerCase())) return false;
  }
...
function buildDataSummary(): string {
  ...
  const overdue      = tasks.filter(t => { const d = daysUntil(String(t['dueDate'] || '')); return d !== null && d < 0 && t['status'] !== 'Done'; }).length;
...
export async function execTool(tool: string, args: AnyRecord): Promise<unknown> {
  switch (tool) {
    ...
    case 'query_records': {
      const store = String(args['store'] || '');
      if (!_SCHEMAS[store]) throw new Error(`Unknown store: ${store}`);
      return (getStore(store) as AnyRecord[]).filter(r => matchesFilter(r, store, args['filter'] as AnyRecord | null));
    }
    case 'count_records': {
      const store = String(args['store'] || '');
      if (!_SCHEMAS[store]) throw new Error(`Unknown store: ${store}`);
      return { count: (getStore(store) as AnyRecord[]).filter(r => matchesFilter(r, store, args['filter'] as AnyRecord | null)).length };
    }
    case 'summarize_record': {
      ...
      if (store === 'projects') extras['tasks']        = (getStore('tasks')    as AnyRecord[]).filter(t => t['projectId']  === rec['id']);
      if (store === 'clients')  extras['projects']     = (getStore('projects') as AnyRecord[]).filter(p => p['clientId']   === rec['id']);
      if (store === 'people')   extras['assignedTasks'] = (getStore('tasks')   as AnyRecord[]).filter(t => t['assigneeId'] === rec['id']);
      return { record: rec, ...extras };
    }
    case 'start_timer': {
      const task = args['taskId_or_name'] ? resolveRecord('tasks', String(args['taskId_or_name'])) : null;
    ...
    case 'get_running_timer': {
      const r    = getRunningTimer() as AnyRecord | null;
      if (!r) return { running: false };
      const task = r['taskId'] ? dbGetById('tasks', String(r['taskId'])) as AnyRecord | null : null;
    ...
    case 'log_communication': {
      const rec: AnyRecord = { type: args['type'] || 'Note', subject: args['subject'] || '', body: args['body'] || '', date: nowISO() };
      if (args['relatedTo']) {
        const rt = args['relatedTo'] as AnyRecord;
        const r  = resolveRecord(String(rt['store']), String(rt['id_or_name']));
        if (r) { rec['relatedStore'] = rt['store']; rec['relatedId'] = r['id']; }
      }
...
export async function speakResult(tool: string, args: AnyRecord, result: unknown, ctx: string): Promise<string> {
  // Simple deterministic cases
  if (tool === 'create_record')   return `Created ${String(args['store'] || 'record').replace(/s$/, '')} "${(result as AnyRecord)?.['title'] || (result as AnyRecord)?.['name'] || (result as AnyRecord)?.['id']}".`;
  if (tool === 'update_record')   return `Updated ${args['store']}: ${Object.keys((args['changes'] as AnyRecord) || {}).join(', ')} set.`;
  if (tool === 'delete_record')   return 'Moved to Recycle Bin.';
  if (tool === 'navigate')        return `Opening ${args['view']}…`;
  if (tool === 'start_timer')     return `Timer started${args['taskId_or_name'] ? ` on "${args['taskId_or_name']}"` : ''}.`;
  if (tool === 'stop_timer')      return (result as AnyRecord)?.['running'] === false ? 'No timer was running.' : 'Timer stopped.';
  if (tool === 'count_records')   return `${(result as AnyRecord)?.['count']} ${args['store']}${args['filter'] ? ' matching filter' : ''}.`;
  ...
  if (tool === 'get_running_timer') {
    ...
    return r?.['running'] ? `Timer running${r['task'] ? ` on "${r['task']}"` : ''} since ${r['since']}.` : 'No timer running.';
  }
  if (tool === 'list_files') {
  ...
  const ctxData  = tool === 'answer_question'
  ...
  try {
    ...
    if (text) return text;
  } catch (e) { console.warn('[AI] speakResult fallback:', (e as Error).message); }
