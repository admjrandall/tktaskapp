...
import { loadOllama, callOllama, fetchOllamaModels as _fetchOllamaModels } from './providers/ollama.js';
...
let _trackUsage: (prov: string, model: string, tokensIn: number, tokensOut: number) => void = () => {};
export function setRuntimeUsageTracker(fn: typeof _trackUsage): void {
  _trackUsage = fn;
}

// ── Model catalog helpers ──────────────────────────────────────────────────────
export const STATIC_MODELS: Array<{ id: string; backend: string; model: string | null; label: string; size: string }> = [
  { id: 'nano', backend: 'nano', model: null, label: 'Gemini Nano', size: 'built-in' },
  ...(__OT_ONLY_BUILD__ ? [] : [
    { id: 'webllm-e2b', backend: 'webllm', model: 'onnx-community/gemma-4-E2B-it-ONNX', label: 'Gemma 4 E2B', size: '~500MB' },
    { id: 'webllm-e4b', backend: 'webllm', model: 'onnx-community/gemma-4-E4B-it-ONNX', label: 'Gemma 4 E4B', size: '~1.5GB' },
  ]),
];
...
export function activeModelId(): string | null {
  ...
  if (aiRuntime.backend === 'webllm') {
    const hit = STATIC_MODELS.find(m => m.id === `webllm-${aiPrefs.browser.modelId.replace(/^gemma-/, '')}`);
...
export async function selectModel(entry: typeof STATIC_MODELS[0]): Promise<void> {
  ...
  const wasOnAIView = (getState() as AppState).currentView === 'ai';
  if (entry.backend === 'nano') {
    aiPrefs.tier = 'browser';
    aiPrefs.browser.modelId = 'nano';
  } else if (entry.backend === 'webllm') {
    aiPrefs.tier = 'browser';
    aiPrefs.browser.modelId = /E4B/i.test(String(entry.model || '')) ? 'gemma-e4b' : 'gemma-e2b';
  ...
  if (wasOnAIView) _appRenderWorkspace('ai'); else _fullRender(getState() as AppState);
...
export async function startAILoad(): Promise<void> {
  ...
  try {
    const { getState } = await import('../state.js');
    _appRenderWorkspace((getState() as AppState).currentView);
  ...
  try {
    const { getState } = await import('../state.js');
    const _state = getState() as AppState;
...
export async function disconnectAI(): Promise<void> {
  ...
  if (aiRuntime.abortController) {
    try { aiRuntime.abortController.abort(); } catch { /* already aborted */ }
...
async function callCloud(
  ...
  if (prov === 'anthropic') {
    const { text, tokensIn, tokensOut } = await callAnthropic({ key, model, system: systemPrompt, history, onToken, ...signalOpt });
    _trackUsage('anthropic', model, tokensIn, tokensOut);
    return text;
  }
  if (prov === 'openai') {
    const { text, tokensIn, tokensOut } = await callOpenAI({ key, model, system: systemPrompt, history, onToken, ...signalOpt });
    _trackUsage('openai', model, tokensIn, tokensOut);
    return text;
  }
  if (prov === 'google') {
    const { text, tokensIn, tokensOut } = await callGoogle({ key, model, system: systemPrompt, history, onToken, ...signalOpt });
