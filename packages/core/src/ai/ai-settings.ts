...
import {
  ...
  startAILoad,
  disconnectAI,
  setRuntimeSecretsGetter,
  setRuntimeUsageTracker,
  ...
  ? [
      {
        id: 'nano',
        label: 'Gemini Nano',
        size: '~4 GB (Chrome-managed)',
        desc: 'Chrome Built-in AI. Requires Chrome 127+ with the Prompt API flag enabled.',
        hf: '',
      },
    ]
  : [
      {
        ...
        label: 'Gemini Nano',
        size: '~4 GB (Chrome-managed)',
        desc: 'Chrome Built-in AI. Requires Chrome 127+ with prompt-api flag.',
        hf: '',
      },
      {
        id: 'gemma-e2b',
        ...
        desc: 'Default in-browser model. Good quality, runs on most WebGPU laptops.',
        hf: 'onnx-community/gemma-4-E2B-it-ONNX',
      },
      {
        id: 'gemma-e4b',
        label: 'Gemma 4 E4B',
        size: '~1.5 GB download',
        desc: 'Higher quality. Needs ~4 GB VRAM.',
        hf: 'onnx-community/gemma-4-E4B-it-ONNX',
      },
    ]
...
export const OLLAMA_CATALOG = [
  {
    ...
    size: '1.9 GB',
    ram: '8 GB',
    why: 'Default. Strong tool-calling, fast.',
  },
  {
    id: 'oll-llama32-3b',
    tag: 'llama3.2:3b',
    size: '2.0 GB',
    ram: '8 GB',
    why: 'Familiar Meta model, solid general chat.',
  },
  {
    id: 'oll-gemma3-1b',
    tag: 'gemma3:1b',
    size: '815 MB',
    ram: '4 GB',
    why: 'For low-end hardware or quick trials.',
  },
  {
    id: 'oll-llama32-1b',
    tag: 'llama3.2:1b',
    size: '1.3 GB',
    ram: '4 GB',
    why: 'Snappiest option, weaker reasoning.',
  },
  {
    id: 'oll-gemma3-4b',
  ...
  },
  {
    ...
    size: '4.7 GB',
    ram: '16 GB',
    why: 'Step-up reasoning, multilingual.',
  },
  {
    id: 'oll-llama31-8b',
    tag: 'llama3.1:8b',
    size: '4.7 GB',
    ram: '16 GB',
    why: 'Strong all-rounder.',
  },
  {
    id: 'oll-qwen25-coder-7b',
  ...
  {
    id: 'oll-deepseek-r1-7b',
    tag: 'deepseek-r1:7b',
    size: '4.7 GB',
    ram: '16 GB',
    why: 'Best small local reasoner.',
  },
  {
    id: 'oll-mistral-7b',
    tag: 'mistral:7b',
        ...
        models: [
          {
            ...
            priceIn: 15.0,
            priceOut: 75.0,
            note: 'Flagship reasoning and coding.',
          },
          {
      ...
      openai: {
        ...
        models: [
          ...
          },
          {
            ...
            priceIn: 0.25,
            priceOut: 2.0,
            note: 'Cost-effective production default.',
          },
        ],
      },
      google: {
        ...
        models: [
          {
            ...
            priceIn: 2.0,
            priceOut: 12.0,
            note: 'Preview flagship reasoning.',
          },
          {
...
setRuntimeCloudLabelGetters(
  (prov) => {
    const m = CLOUD_PROVIDERS[prov]?.models.find(
      (x) => x.id === aiPrefs.cloud.modelByProvider[prov],
    )
    return m?.label || ''
  },
...
export async function aiSecretsSave(obj: Record<string, string>): Promise<void> {
  ...
  await _idbPutRecord('documents', {
    id: AI_SECRETS_IDB_KEY,
    secrets: obj || {},
    updatedAt: nowISO(),
  })
...
export async function aiSecretsWipe(): Promise<void> {
  try {
    await _idbDeleteRecord('documents', AI_SECRETS_IDB_KEY)
  } catch {
    /* already gone */
  }
...
export function aiTrackUsage(
  ...
  if (modelDef) {
    aiPrefs.cloud.usage.estCostUsd +=
      ((tokensIn || 0) * modelDef.priceIn) / 1e6 + ((tokensOut || 0) * modelDef.priceOut) / 1e6
  }
...
function _getNanoAPI(): Record<string, unknown> | undefined {
  ...
  return (w['LanguageModel'] ??
    (w['ai'] as Record<string, unknown> | undefined)?.['languageModel']) as
...
export async function openNanoDownloadModal(): Promise<void> {
  ...
  try {
    ...
    let avail: string
    try {
      avail = await (api['availability'] as (o: unknown) => Promise<string>)({
...
function _corsBlock(os: string): { title: string; cmd: string; after: string } {
  ...
  if (os === 'mac')
    return {
      ...
      after: 'Then quit Ollama from the menu bar (Cmd+Q) and relaunch.',
    }
  if (os === 'win')
    return {
      title: 'Windows',
      cmd: `setx OLLAMA_ORIGINS "${origin}"`,
      after: 'Then right-click the Ollama tray icon → Quit, and relaunch Ollama.',
    }
  if (os === 'linux')
    return {
      title: 'Linux (systemd)',
  ...
  return {
    ...
    after: 'Set the env var, then run ollama serve.',
  }
}
...
export function renderAIWizard(): string {
  ...
  if (w.step === 1) body = renderWizardStep1()
    ...
    if (!isAITierAllowed(w.tier))
      body = `<div class="card" style="padding:1rem;border-color:#fecaca;color:#991b1b">This AI tier is not allowed in the ${escH(deploymentPolicy.label)} build.</div>`
  ...
  const back =
    w.step > 1
      ? `<button class="btn btn-secondary btn-sm" id="aiw-back">← Back</button>`
      : `<span></span>`
...
function renderWizardActions(): string {
  ...
  if (w.step === 2)
    return `<span style="font-size:.75rem;color:var(--text-tertiary)">Pick a tier to continue</span>`
  if (w.step === 3) {
  ...
  if (w.step === 4)
    return `<button class="btn btn-primary btn-sm" id="aiw-finish">Start chatting</button>`
  return ''
}
...
function renderWizardStep2(): string {
  ...
  const cards: string[] = []
  if (isAITierAllowed('browser'))
    cards.push(
      card(
        'browser',
        'In-browser',
        'Try it',
        ['No install, works offline after one download', 'Fully private', 'Gemma 4 or Gemini Nano'],
        ['Smaller models', 'Needs WebGPU'],
      ),
    )
  if (isAITierAllowed('ollama'))
    cards.push(
      card(
        'ollama',
        isOTOnlyMode() ? 'Local/Internal AI Server' : 'Ollama',
        isOTOnlyMode() ? 'OT allowed' : 'Power user',
        ['Local or internal LAN endpoint', 'No cloud provider account', 'Ollama-compatible API'],
        isOTOnlyMode()
          ? ['Endpoint must be allowlisted in CSP', 'Models must be provisioned outside the app']
          : ['Install + one-time CORS config'],
      ),
    )
  if (isAITierAllowed('cloud'))
    cards.push(
      card(
        'cloud',
        'Cloud',
        'Best quality',
        ['Claude / GPT / Gemini frontier quality', 'No hardware needs', 'Fastest responses'],
        ['Data leaves device', 'Pay per token', 'API key required'],
      ),
    )
...
function renderWizardStep3Browser(): string {
  ...
  const hasNano = !!(
    ...
    ((window as unknown as AnyRecord)['ai'] as AnyRecord | undefined)?.['languageModel']
  )
  ...
  const opt = (
    id: string,
    m: (typeof BROWSER_MODELS)[0],
  ) => `<div class="aiw-bm" data-aiw-bm="${id}" style="border:2px solid ${w.draft.browserModelId === id ? 'var(--accent)' : 'var(--border-subtle)'};border-radius:var(--radius-md);padding:.75rem 1rem;cursor:pointer;display:flex;gap:.75rem;align-items:flex-start;background:${w.draft.browserModelId === id ? 'var(--bg-base)' : 'transparent'}">
...
function renderWizardStep3Ollama(): string {
  ...
  const probeChip = (() => {
    ...
    if (probe.state === 'ok')
      return `<span style="font-size:.75rem;color:#10b981;font-weight:600">✓ Reachable (Ollama ${escH(probe.version || '')}, ${probe.models?.length || 0} models)</span>`
    if (probe.state === 'cors')
      return `<span style="font-size:.75rem;color:#f59e0b;font-weight:600">⚠ CORS-blocked — complete step 2 and restart Ollama</span>`
    if (probe.state === 'down')
      return `<span style="font-size:.75rem;color:#dc2626;font-weight:600">✗ Not running — start Ollama from your menu/tray</span>`
    if (probe.state === 'badurl')
      return `<span style="font-size:.75rem;color:#dc2626;font-weight:600">✗ Bad URL</span>`
    return ''
  })()
  const pulled = new Set(probe?.models || [])
  const fileNote =
    location.protocol === 'file:'
      ? `<div style="font-size:.75rem;color:#78350f;background:#fef3c7;padding:.5rem .625rem;border-radius:var(--radius-sm);margin-top:.5rem">You opened this app via <code>file://</code>. Use <code>OLLAMA_ORIGINS=null</code> (literal string) or serve the file through a local web server.</div>`
      : ''
  ...
  return `<h3 style="font-size:1.125rem;font-weight:600;margin:0 0 .25rem">Set up local/internal AI</h3>
    ...
    <details ${probe?.state === 'ok' ? 'open' : ''} style="border:1px solid var(--border-subtle);border-radius:var(--radius-md);padding:.625rem .75rem">
      ...
      <div style="margin-top:.5rem;font-size:.8125rem">
        ${
          probe?.state === 'ok'
            ? `<div style="display:flex;flex-direction:column;gap:.375rem;max-height:300px;overflow-y:auto">${OLLAMA_CATALOG.map(
                (m) => {
        ...
        ${
          allowPull
            ? `<div style="display:flex;gap:.5rem;margin-top:.625rem;align-items:center">
          ...
          <button class="btn btn-secondary btn-sm" id="aiw-ollama-other-pull">${Icons.Download(14)} Pull</button>
            ...
            : `<div style="font-size:.75rem;color:var(--text-tertiary);margin-top:.625rem;line-height:1.5">Model pulls are disabled in this build. Preload approved models on the local/internal AI server, then select one from the detected list.</div>`
        }`
            : `<div style="color:var(--text-tertiary);font-style:italic">Complete step 3 first.</div>`
        }
      </div>
    </details>`
}
...
function renderWizardStep3Cloud(): string {
  ...
  const seg = `<div style="display:inline-flex;border:1px solid var(--border-subtle);border-radius:var(--radius-md);overflow:hidden;margin-bottom:.875rem">
    ${Object.keys(CLOUD_PROVIDERS)
      .map(
        (p) =>
          `<button class="aiw-cprov" data-aiw-cprov="${p}" style="padding:.5rem .875rem;border:none;background:${p === prov ? 'var(--accent)' : 'transparent'};color:${p === prov ? '#fff' : 'var(--text-primary)'};font-weight:500;font-size:.8125rem;cursor:pointer">${escH(CLOUD_PROVIDERS[p]?.label ?? p)}</button>`,
      )
      .join('')}
  </div>`
  let panel = ''
  if (prov) {
    ...
    ) => `<div class="aiw-cmodel" data-aiw-cmodel="${escH(m.id)}" style="border:1px solid ${w.draft.cloudModelByProvider[prov] === m.id ? 'var(--accent)' : 'var(--border-subtle)'};border-radius:var(--radius-sm);padding:.5rem .625rem;cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:.5rem;background:${w.draft.cloudModelByProvider[prov] === m.id ? 'var(--bg-base)' : 'transparent'}">
...
function renderWizardStep4(): string {
  ...
  let modelLabel = ''
  if (w.tier === 'browser')
    modelLabel = BROWSER_MODELS.find((m) => m.id === w.draft.browserModelId)?.label || ''
...
export function bindAIWizard(): void {
  ...
  document.getElementById('aiw-back')?.addEventListener('click', () => {
    w.step = Math.max(1, w.step - 1)
    _fullRender(getState())
  })
  document.getElementById('aiw-notnow')?.addEventListener('click', closeAIWizard)
  document.getElementById('aiw-continue')?.addEventListener('click', () => {
    w.step = 2
    _fullRender(getState())
  })

  document.querySelectorAll<HTMLElement>('[data-aiw-tier]').forEach((el) => {
  ...
  document.querySelectorAll<HTMLElement>('[data-aiw-bm]').forEach((el) => {
    el.addEventListener('click', () => {
  ...
  document.getElementById('aiw-ollama-test')?.addEventListener('click', async () => {
    const url = (
      document.getElementById('aiw-ollama-url') as HTMLInputElement | null
    )?.value?.trim()
    if (url) w.draft.ollamaUrl = url
    try {
      assertLocalAIEndpointAllowed(w.draft.ollamaUrl)
    } catch (e) {
    ...
    w.testResult = await probeOllama(w.draft.ollamaUrl)
    _fullRender(getState())
  })

  document.querySelectorAll<HTMLElement>('[data-aiw-ollama-use]').forEach((el) => {
    el.addEventListener('click', () => {
  ...
  const startPull = async (tag: string) => {
    ...
    _fullRender(getState())
    try {
      await pullOllamaModel(tag, w.draft.ollamaUrl, (p) => {
        w.pullProgress = { tag, ...p }
        _fullRender(getState())
      })
      w.testResult = await probeOllama(w.draft.ollamaUrl)
  ...
  document.querySelectorAll<HTMLElement>('[data-aiw-ollama-pull]').forEach((el) => {
    el.addEventListener('click', () =>
      startPull((el.dataset as DOMStringMap & { aiwOllamaPull: string }).aiwOllamaPull),
    )
  })
  document.getElementById('aiw-ollama-other-pull')?.addEventListener('click', () => {
    const v = (
      document.getElementById('aiw-ollama-other') as HTMLInputElement | null
    )?.value?.trim()
    if (v) void startPull(v)
    else showToast('Enter a model tag', 'error')
  })

  document.querySelectorAll<HTMLElement>('[data-aiw-cprov]').forEach((el) => {
    el.addEventListener('click', () => {
  ...
  document.getElementById('aiw-cloud-save-key')?.addEventListener('click', async () => {
    ...
    if (provDef?.keyPattern && !provDef.keyPattern.test(k)) {
      showToast(
        `Invalid ${escH(provDef.label)} key format — check the key and try again`,
        'error',
        6000,
      )
      return
  ...
  document.querySelectorAll<HTMLElement>('[data-aiw-cmodel]').forEach((el) => {
    el.addEventListener('click', () => {
      if (w.draft.cloudProvider)
        w.draft.cloudModelByProvider[w.draft.cloudProvider] = (
          el.dataset as DOMStringMap & { aiwCmodel: string }
        ).aiwCmodel
      _fullRender(getState())
    })
  })
  ...
  document.getElementById('aiw-next')?.addEventListener('click', () => {
    w.step = 4
    _fullRender(getState())
  })
