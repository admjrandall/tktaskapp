...
export function migrateAIPrefsV1toV2(v1: AnyRecord): AIPrefs {
  const out = JSON.parse(JSON.stringify(AI_DEFAULT_PREFS)) as AIPrefs
  out.autoApplyCreates = !!v1.autoApplyCreates
  if (v1.backend === 'nano') {
    out.tier = 'browser'
    out.browser.modelId = 'nano'
  ...
  try {
    ...
    if (v2raw) {
      ...
      const merged = {
        ...
        ...parsed,
        browser: {
          ...AI_DEFAULT_PREFS.browser,
          ...(parsed.browser || {}),
          weightsCached: {
            ...AI_DEFAULT_PREFS.browser.weightsCached,
            ...((parsed.browser || {}).weightsCached || {}),
          },
        },
        ollama: { ...AI_DEFAULT_PREFS.ollama, ...(parsed.ollama || {}) },
        cloud: {
          ...AI_DEFAULT_PREFS.cloud,
          ...(parsed.cloud || {}),
          modelByProvider: {
            ...AI_DEFAULT_PREFS.cloud.modelByProvider,
            ...((parsed.cloud || {}).modelByProvider || {}),
          },
      ...
      for (const [provider, fallback] of Object.entries(CURRENT_CLOUD_MODEL_BY_PROVIDER)) {
        if (
          !merged.cloud.modelByProvider[provider] ||
          isRetiredCloudModel(provider, merged.cloud.modelByProvider[provider])
        ) {
    ...
    console.warn('[AI] prefs load failed, using defaults:', (e as Error)?.message)
  }
