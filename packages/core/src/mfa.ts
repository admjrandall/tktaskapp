...
export async function loadTOTPConfig(): Promise<TOTPConfig | null> {
  ...
  try {
    ...
    const rec = records.find(
      (r: Record<string, unknown>) => r['id'] === '__mfa_totp__',
    ) as TOTPConfig | undefined;
    return rec || null;
  } catch {
    return null;
  }
...
export async function generateNewTOTPSecret(): Promise<string> {
  return base32Encode(generateTOTPSecret());
}
