...
const SCHEMAS: Record<string, SchemaDef> = {
  clients: { label: 'Client', fields: [
    ...
    { key: 'stage', label: 'Status', type: 'select', options: ['Prospect', 'Active', 'Inactive', 'Churned'] },
    { key: 'description', label: 'Description', type: 'textarea' },
  ]},
  departments: { label: 'Department', fields: [
  ...
  standaloneNotes: { label: 'Note', fields: [
...
const FILE_ICONS: Record<string, string> = { pdf: '📄', doc: '📝', docx: '📝', xls: '📊', xlsx: '📊', csv: '📊', ppt: '📋', pptx: '📋', png: '🖼️', jpg: '🖼️', jpeg: '🖼️', gif: '🖼️', webp: '🖼️', zip: '🗜️', txt: '📃', mp4: '🎬', mp3: '🎵' };
...
export function renderRecordModal(config: { store: string; id: string | null; defaults?: AnyRecord } | null): string {
  if (!config) return '';
  const { store, id, defaults = {} } = config;
  const schema = SCHEMAS[store]; if (!schema) return '';
  ...
  const fields = schema.fields.map(f => {
    ...
    if (f.type === 'textarea') return `<div class="form-group" style="grid-column:1/-1"><label class="form-label">${f.label}${f.required ? ' <span style="color:#ef4444">*</span>' : ''}</label><textarea class="textarea" name="${f.key}" rows="3">${escH(String(val))}</textarea></div>`;
    if (f.type === 'select') {
    ...
    if (f.type === 'relation') {
      ...
      const opts = rel.map(r => `<option value="${r.id}"${r.id === val ? ' selected' : ''}>${escH(String(r.name || r.title || r.id || ''))}</option>`).join('');
      return `<div class="form-group"><label class="form-label">${f.label}</label><select class="select" name="${f.key}"><option value="">— None —</option>${opts}</select></div>`;
    }
  ...
  const backdropClass = isProjectMobile ? 'modal-backdrop modal-backdrop-fullscreen' : 'modal-backdrop';
  const modalClass = isProjectMobile ? 'modal modal-lg modal-fullscreen' : 'modal modal-lg';

  const filesSection = id ? (() => {
    const recFiles = (dbGetAll('files') as AnyRecord[]).filter(f => f.relatedStore === store && f.relatedId === id);
  ...
  const notesSection = store !== 'standaloneNotes' ? `<div style="margin-top:1.25rem;border-top:1px solid var(--border-subtle);padding-top:1rem"><div style="font-size:.8rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;color:var(--text-tertiary);margin-bottom:.5rem">Notes</div><div id="notes-list">${notes.slice(0, 5).map(n => `<div style="padding:.625rem;background:var(--bg-base);border-radius:var(--radius-md);margin-bottom:.5rem"><div style="font-size:.8rem;line-height:1.5">${escH(String(n.text || ''))}</div><div style="font-size:.7rem;color:var(--text-tertiary);margin-top:.2rem">${formatRelative(String(n.date || ''))}</div></div>`).join('') || '<p style="font-size:.8rem;color:var(--text-tertiary)">No notes</p>'}</div><div style="display:flex;gap:.5rem;margin-top:.5rem"><textarea class="textarea" id="new-note" placeholder="Add a note…" rows="2" style="font-size:.8rem;flex:1"></textarea><button class="btn btn-secondary btn-sm" id="add-note-btn" style="align-self:flex-end">${Icons.Plus(14)}</button></div></div>` : '';
...
export function bindRecordModal(config: { store: string; id: string | null; defaults?: AnyRecord }): void {
  const { store, id, defaults = {} } = config;
  const schema = SCHEMAS[store]; if (!schema) return;
  const record = id ? dbGetById(store, id) as AnyRecord | null : null;
  ...
  document.getElementById('record-modal-backdrop')?.addEventListener('click', (e: Event) => { if ((e.target as HTMLElement).id === 'record-modal-backdrop') close(); });
  ...
  document.getElementById('add-note-btn')?.addEventListener('click', () => {
    ...
    if (list) list.innerHTML += `<div style="padding:.625rem;background:var(--bg-base);border-radius:var(--radius-md);margin-bottom:.5rem"><div style="font-size:.8rem;line-height:1.5">${escH(text)}</div><div style="font-size:.7rem;color:var(--text-tertiary);margin-top:.2rem">just now</div></div>`;
    if (ta) ta.value = '';
  });
  ...
  document.getElementById('delete-record-btn')?.addEventListener('click', () => {
    showConfirm(`Delete this ${schema.label}? It will be moved to the Recycle Bin.`, async () => {
      ...
      reloadData(); showToast(`${schema.label} moved to Recycle Bin`, 'success'); close();
    });
  });
  ...
  if (id) {
    const handleModalFiles = async (files: File[]) => {
      ...
      for (const file of files) {
        ...
        try {
          ...
          await dbCreate('files', { name: file.name, size: file.size, type: file.type, dataUrl, relatedStore: store, relatedId: id, addedAt: nowISO() });
          reloadData();
          showToast(`${file.name} attached`, 'success');
          const recFiles = (dbGetAll('files') as AnyRecord[]).filter(f => f.relatedStore === store && f.relatedId === id);
          const listEl = document.getElementById('modal-files-list');
          if (listEl) listEl.innerHTML = recFiles.length ? recFiles.map(renderModalFileRow).join('') : '<p style="font-size:.8rem;color:var(--text-tertiary)">No files attached</p>';
          document.querySelectorAll<HTMLElement>('[data-modal-del-file]').forEach(btn => btn.addEventListener('click', () => showConfirm('Remove this file?', async () => { await dbDelete('files', (btn.dataset as DOMStringMap & { modalDelFile: string }).modalDelFile); reloadData(); btn.closest('div[style]')?.remove(); })));
    ...
    fileInput?.addEventListener('change', async e => { await handleModalFiles(Array.from((e.target as HTMLInputElement).files || [])); (e.target as HTMLInputElement).value = ''; });
    document.querySelectorAll<HTMLElement>('[data-modal-del-file]').forEach(btn => btn.addEventListener('click', () => showConfirm('Remove this file?', async () => { await dbDelete('files', (btn.dataset as DOMStringMap & { modalDelFile: string }).modalDelFile); reloadData(); _appRenderWorkspace(getState().currentView); })));
  }

  document.getElementById('modal-save')?.addEventListener('click', async () => {
    const form = document.getElementById('record-form') as HTMLFormElement | null; if (!form) return;
    const fd: AnyRecord = {};
    schema.fields.forEach(f => {
      if (f.type === 'multirelation') {
        ...
        const el = form.querySelector<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(`[name="${f.key}"]`);
        fd[f.key] = el ? sanitize(el.value) : '';
      }
    });
    fd.tagIds = [...form.querySelectorAll<HTMLInputElement>('[name="tagIds"]:checked')].map(c => c.value);
    fd.notes = _pendingNotes;
    for (const f of schema.fields) {
      if (f.required && !fd[f.key]) { showToast(`${f.label} is required`, 'error'); return; }
    }
    try {
      if (record) { await dbUpdate(store, id!, fd); showToast(`${schema.label} updated`, 'success'); }
      else {
        if (store === 'standaloneNotes') fd.createdAt = new Date().toISOString();
        await dbCreate(store, fd); showToast(`${schema.label} created`, 'success');
      }
      reloadData(); close();
