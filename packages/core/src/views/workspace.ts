...
import { renderViewTabs, renderSearchInput } from '../components.js';
import { PRIORITIES, PROJECT_STAGES, TASK_STATUSES } from '../constants.js';
import { openProjectCanvas } from './project-canvas.js';
import {
  renderListView, bindListView,
  renderGridView, bindGridView,
  renderKanbanView, bindKanbanView,
  renderSpatialCanvas, bindSpatialCanvas,
...
function wsState(store: string): WSState {
  if (!WS_STATE[store]) WS_STATE[store] = { view: 'list', search: '', sortField: 'updatedAt', sortDir: 'desc', filters: {}, filterOpen: false };
  return WS_STATE[store]!;
}

const WS_META: Record<string, { label: string; searchFields: string[]; stageField?: string; stages?: string[] }> = {
  clients:       { label: 'Clients',      searchFields: ['name', 'contactName', 'email', 'phone'], stageField: 'stage', stages: ['Prospect', 'Active', 'Inactive', 'Churned'] },
  departments:   { label: 'Departments',  searchFields: ['name', 'description'] },
  projects:      { label: 'Projects',     searchFields: ['name', 'description'], stageField: 'stage', stages: PROJECT_STAGES },
  tasks:         { label: 'Tasks',        searchFields: ['title', 'description'], stageField: 'status', stages: TASK_STATUSES },
...
export function renderWorkspaceView(store: string): string {
  const meta = WS_META[store]; if (!meta) return `<div style="padding:2rem">Unknown: ${store}</div>`;
  ...
  const onOpen = (id: string) => { if (store === 'projects') openProjectCanvas(id); else openRecordModal(store, id); };
  const onRefresh = () => _appRenderWorkspace(store);

  let body = '';
  if (s.view === 'list') body = renderListView(store, records, onOpen, s.sortField, s.sortDir, f => { if (s.sortField === f) s.sortDir = s.sortDir === 'asc' ? 'desc' : 'asc'; else { s.sortField = f; s.sortDir = 'asc'; } _appRenderWorkspace(store); });
  ...
  const filterPanel = s.filterOpen ? `<div class="filter-panel" style="position:absolute;top:calc(100%+6px);right:0;z-index:50;min-width:240px" id="filter-panel">
...
export function bindWorkspaceView(store: string): void {
  const s = wsState(store); const meta = WS_META[store];
  if (!meta) return;
  let records = sortRecords(filterRecords(searchRecords(dbGetAll(store) as AnyRecord[], s.search, meta.searchFields), s.filters), s.sortField, s.sortDir);
  document.querySelectorAll<HTMLElement>('[data-view]').forEach(btn => btn.addEventListener('click', () => { s.view = (btn.dataset as DOMStringMap & { view: string }).view; _appRenderWorkspace(store); }));
  ...
  if (si) {
    si.addEventListener('input', e => {
      ...
      if (!body) { _appRenderWorkspace(store); return; }
      ...
      const oOpen = (id: string) => { if (store === 'projects') openProjectCanvas(id); else openRecordModal(store, id); };
      const oRefresh = () => _appRenderWorkspace(store);
      const oSort = (f: string) => { if (s.sortField === f) s.sortDir = s.sortDir === 'asc' ? 'desc' : 'asc'; else { s.sortField = f; s.sortDir = 'asc'; } _appRenderWorkspace(store); };
      if (s.view === 'list') { body.innerHTML = renderListView(store, r, oOpen, s.sortField, s.sortDir, oSort); bindListView(oOpen, oSort); }
      else if (s.view === 'grid') { body.innerHTML = renderGridView(store, r, oOpen); bindGridView(oOpen); }
      else if (s.view === 'kanban') { body.innerHTML = renderKanbanView(store, r, oOpen); bindKanbanView(store, r, oOpen, oRefresh); }
      else if (s.view === 'spatial') { body.innerHTML = renderSpatialCanvas(store, r, oOpen); bindSpatialCanvas(store, r, oOpen, oRefresh); }
    });
  }

  document.getElementById('ws-add-btn')?.addEventListener('click', () => openRecordModal(store));
  document.getElementById('ws-export-btn')?.addEventListener('click', () => {
    ...
    const fields = Object.keys(records[0]!).filter(k => !k.startsWith('_') && k !== 'files' && k !== 'notes');
    downloadText(`taskapp-${store}.csv`, toCSV(records, fields), 'text/csv');
    showToast(`Exported ${records.length} records`, 'success');
  });
  document.getElementById('ws-filter-btn')?.addEventListener('click', (e: Event) => { e.stopPropagation(); s.filterOpen = !s.filterOpen; _appRenderWorkspace(store); });
  if (s.filterOpen) {
    document.getElementById('filter-apply')?.addEventListener('click', () => { document.querySelectorAll<HTMLSelectElement>('[data-filter]').forEach(el => { s.filters[(el.dataset as DOMStringMap & { filter: string }).filter] = el.value; }); s.filterOpen = false; _appRenderWorkspace(store); });
    document.getElementById('filter-clear')?.addEventListener('click', () => { s.filters = {}; s.filterOpen = false; _appRenderWorkspace(store); });
    setTimeout(() => {
      document.addEventListener('pointerdown', function cf(e) {
        if (!(e.target as HTMLElement).closest('#filter-panel') && !(e.target as HTMLElement).closest('#ws-filter-btn')) {
          s.filterOpen = false; document.removeEventListener('pointerdown', cf); _appRenderWorkspace(store);
        }
      });
    }, 0);
  }
  const onOpen = (id: string) => { if (store === 'projects') openProjectCanvas(id); else openRecordModal(store, id); };
  const onRefresh = () => _appRenderWorkspace(store);
  const onSort = (f: string) => { if (s.sortField === f) s.sortDir = s.sortDir === 'asc' ? 'desc' : 'asc'; else { s.sortField = f; s.sortDir = 'asc'; } _appRenderWorkspace(store); };
