...
export function renderCalendar(state: AppState): string {
  ...
  const events: CalEvent[] = [
    ...(tasks as Record<string, unknown>[]).filter(t => t.dueDate).map(t => ({ date: String(t.dueDate), label: String(t.title || 'Task'), color: '#6366f1' })),
    ...(projects as Record<string, unknown>[]).filter(p => p.dueDate).map(p => ({ date: String(p.dueDate), label: String(p.name || 'Project'), color: '#10b981' })),
  ];
  const viewBtns = ['month', 'week', 'day', 'agenda'].map(v => `<button class="btn btn-sm ${_calView === v ? 'btn-primary' : 'btn-secondary'}" data-calview="${v}">${v.charAt(0).toUpperCase() + v.slice(1)}</button>`).join('');
  let body = '';
  if (_calView === 'month') {
    const yr = _calDate.getFullYear(), mo = _calDate.getMonth();
    const first = new Date(yr, mo, 1); const last = new Date(yr, mo + 1, 0); const sd = first.getDay();
    const today = new Date().toDateString();
    let cells = '';
    for (let i = 0; i < sd; i++) { const d = new Date(yr, mo, -sd + i + 1); cells += `<div class="cal-day other-month"><div class="cal-day-num">${d.getDate()}</div></div>`; }
    for (let d = 1; d <= last.getDate(); d++) {
      const date = new Date(yr, mo, d); const ds = date.toISOString().split('T')[0];
      const isT = date.toDateString() === today; const de = events.filter(e => e.date === ds);
      cells += `<div class="cal-day${isT ? ' today' : ''}"><div class="cal-day-num">${d}</div>${de.slice(0, 3).map(e => `<div class="cal-event" style="background:${e.color}22;color:${e.color};border-left:2px solid ${e.color}">${escH(e.label)}</div>`).join('')}${de.length > 3 ? `<div style="font-size:.6rem;color:var(--text-tertiary)">+${de.length - 3}</div>` : ''}</div>`;
    }
...
export function bindCalendar(): void {
  document.getElementById('cal-prev')?.addEventListener('click', () => { if (_calView === 'month') _calDate.setMonth(_calDate.getMonth() - 1); else if (_calView === 'week') _calDate.setDate(_calDate.getDate() - 7); else _calDate.setDate(_calDate.getDate() - 1); _appRenderWorkspace('calendar'); });
  document.getElementById('cal-next')?.addEventListener('click', () => { if (_calView === 'month') _calDate.setMonth(_calDate.getMonth() + 1); else if (_calView === 'week') _calDate.setDate(_calDate.getDate() + 7); else _calDate.setDate(_calDate.getDate() + 1); _appRenderWorkspace('calendar'); });
  document.getElementById('cal-today')?.addEventListener('click', () => { _calDate = new Date(); _appRenderWorkspace('calendar'); });
  document.querySelectorAll<HTMLElement>('[data-calview]').forEach(b => b.addEventListener('click', () => { _calView = (b.dataset as DOMStringMap & { calview: string }).calview; _appRenderWorkspace('calendar'); }));
}
