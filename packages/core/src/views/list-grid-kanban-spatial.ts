...
import { renderEmpty, renderPriorityBadge, renderDueBadge } from '../components.js';
import { Icons } from '../icons.js';

type AnyRecord = Record<string, unknown>;

export function stageBadge(s: unknown): string {
  const m: Record<string, string> = { Lead: 'badge-slate', Active: 'badge-indigo', Review: 'badge-amber', 'On Hold': 'badge-slate', Done: 'badge-green', Cancelled: 'badge-rose' };
  return s ? `<span class="badge ${m[String(s)] || 'badge-slate'}">${s}</span>` : '';
}
export function statusBadge(s: unknown): string {
  const m: Record<string, string> = { Todo: 'badge-slate', 'In Progress': 'badge-indigo', Blocked: 'badge-rose', Done: 'badge-green' };
  return s ? `<span class="badge ${m[String(s)] || 'badge-slate'}">${s}</span>` : '';
}
...
export function renderListView(
  ...
  const colMap: Record<string, [string, string][]> = {
    clients: [['name', 'Name'], ['contactName', 'Contact'], ['email', 'Email'], ['updatedAt', 'Updated']],
    departments: [['name', 'Name'], ['description', 'Description'], ['updatedAt', 'Updated']],
    projects: [['name', 'Name'], ['stage', 'Stage'], ['priority', 'Priority'], ['dueDate', 'Due'], ['updatedAt', 'Updated']],
    tasks: [['title', 'Title'], ['status', 'Status'], ['priority', 'Priority'], ['dueDate', 'Due'], ['updatedAt', 'Updated']],
    people: [['name', 'Name'], ['role', 'Role'], ['email', 'Email'], ['updatedAt', 'Updated']],
    standaloneNotes: [['body', 'Note'], ['createdAt', 'Created'], ['updatedAt', 'Updated']],
  };
  const cols = colMap[store] || [['name', 'Name'], ['updatedAt', 'Updated']];
  if (!records.length) return renderEmpty(Icons.List(48), 'No records yet', 'Create one to get started.');
  ...
  const heads = cols.map(([field, label]) => {
...
export function bindListView(onOpen: (id: string) => void, onSort: (f: string) => void): void {
  document.querySelectorAll<HTMLTableRowElement>('.list-table tbody tr').forEach(tr => tr.addEventListener('click', () => onOpen(tr.dataset.id!)));
  document.querySelectorAll<HTMLTableCellElement>('.list-table th[data-sort]').forEach(th => th.addEventListener('click', () => onSort(th.dataset.sort!)));
}

export function renderGridView(store: string, records: AnyRecord[], onOpen: (id: string) => void): string {
  if (!records.length) return renderEmpty(Icons.Grid(48), 'No records yet', 'Create one to get started.');
  return `<div class="grid-view">${records.map(r => {
    const name = store === 'standaloneNotes' ? (String(r.body || '').slice(0, 20) || 'Note') : String(r.name || r.title || 'Untitled');
    const [bg, fg] = avatarColor(name);
    let meta = '';
    if (store === 'clients') meta = `<div class="text-xs text-secondary mt-1">${escH(String(r.contactName || 'No contact'))}</div>`;
    if (store === 'departments') meta = `<div class="text-xs text-secondary mt-1">${escH(String(r.description || ''))}</div>`;
    if (store === 'projects') meta = `<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.5rem">${stageBadge(r.stage)} ${renderPriorityBadge(String(r.priority || ''))}</div>`;
    if (store === 'tasks') meta = `<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.5rem">${statusBadge(r.status)} ${renderPriorityBadge(String(r.priority || ''))}</div>`;
    if (store === 'standaloneNotes') {
      const preview = String(r.body || '').replace(/\n/g, ' ').slice(0, 60);
...
export function bindGridView(onOpen: (id: string) => void): void {
  document.querySelectorAll<HTMLElement>('.grid-card').forEach(c => c.addEventListener('click', () => onOpen(c.dataset.id!)));
}
...
export function renderKanbanView(store: string, records: AnyRecord[], onOpen: (id: string) => void): string {
  let cols: string[], sf: string;
  if (store === 'standaloneNotes') {
    const allTags = dbGetAll('tags') as AnyRecord[];
    const usedTagIds = [...new Set(records.flatMap(r => Array.isArray(r.tagIds) ? (r.tagIds as string[]) : []))];
    const tagCols = usedTagIds.map(id => allTags.find(t => t.id === id)?.name as string).filter(Boolean);
  ...
  return `<div class="kanban-board">${cols.map(col => {
    const cr = store === 'standaloneNotes'
      ? (col === 'Untagged'
        ? records.filter(r => !Array.isArray(r.tagIds) || (r.tagIds as unknown[]).length === 0)
        : records.filter(r => { const tag = (dbGetAll('tags') as AnyRecord[]).find(t => t.name === col); return tag && Array.isArray(r.tagIds) && (r.tagIds as string[]).includes(String(tag.id)); }))
      : records.filter(r => (r[sf] || cols[0]) === col);
    const cards = cr.map(r => {
      const name = store === 'standaloneNotes' ? (String(r.body || '').slice(0, 30) || 'Note') : String(r.name || r.title || 'Untitled');
      const [bg, fg] = avatarColor(name);
      const cardBody = store === 'standaloneNotes'
...
export function bindKanbanView(store: string, records: AnyRecord[], onOpen: (id: string) => void, onRefresh: () => void): void {
  ...
  document.querySelectorAll<HTMLElement>('.kanban-col').forEach(col => {
    ...
    col.addEventListener('drop', async e => {
      e.preventDefault(); col.classList.remove('drag-over'); if (!dragging) return;
      const ns = col.dataset.col!;
      const rec = records.find(r => r.id === dragging);
      if (rec && rec[sf] !== ns) { await dbUpdate(store, dragging!, { [sf]: ns }); onRefresh(); }
      dragging = null;
    });
  });
}

export function renderSpatialCanvas(store: string, records: AnyRecord[], onOpen: (id: string) => void): string {
  if (!records.length) return renderEmpty(Icons.Spatial(48), 'No records yet', 'Create one to place it on the canvas.');
  const nodes = records.map(r => {
    ...
    const content = store === 'standaloneNotes'
...
export function bindSpatialCanvas(store: string, records: AnyRecord[], onOpen: (id: string) => void, _onRefresh: () => void): void {
  ...
  document.querySelectorAll<HTMLElement>('.spatial-node').forEach(node => {
    ...
    inner.addEventListener('click', () => { if (!dragging) onOpen(node.dataset.id!); });
    inner.addEventListener('mousedown', (e: MouseEvent) => {
      if (e.button !== 0) return;
      dragging = false; startX = e.clientX; startY = e.clientY; origL = node.offsetLeft; origT = node.offsetTop;
      const w = (canvas as HTMLElement).offsetWidth || 1, h = (canvas as HTMLElement).offsetHeight || 1;
      ...
      const onUp = async () => {
        ...
        if (!dragging) return;
        await dbUpdate(store, node.dataset.id!, { _x: parseFloat(node.style.left), _y: parseFloat(node.style.top) });
      };
