...
import { dbGetAll, dbGetById, dbUpdate, softDelete } from '../db.js';
...
export function setPcFromCanvas(val: boolean): void { _pcFromCanvas = val; }
...
export async function pcSaveLayout(): Promise<void> {
  ...
  try { await dbUpdate('projects', _pcProjectId, { _canvasLayout: layout }); } catch (e) { console.warn('layout save failed', e); }
}
...
export function pcDetailsBody(project: AnyRecord): string {
  const clientOpts = (dbGetAll('clients') as AnyRecord[]).map(c => `<option value="${c.id}"${c.id === project.clientId ? ' selected' : ''}>${escH(String(c.name || ''))}</option>`).join('');
  const ownerOpts = (dbGetAll('people') as AnyRecord[]).map(p => `<option value="${p.id}"${p.id === project.ownerId ? ' selected' : ''}>${escH(String(p.name || ''))}</option>`).join('');
  const deptOpts = (dbGetAll('departments') as AnyRecord[]).map(d => `<option value="${d.id}"${d.id === project.deptId ? ' selected' : ''}>${escH(String(d.name || ''))}</option>`).join('');
  const stageOpts = PROJECT_STAGES.map(s => `<option value="${s}"${s === project.stage ? ' selected' : ''}>${s}</option>`).join('');
  const prioOpts = PRIORITIES.map(p => `<option value="${p}"${p === project.priority ? ' selected' : ''}>${p}</option>`).join('');
  return `
...
export function pcBindDetails(project: AnyRecord): void {
  document.querySelectorAll<HTMLInputElement | HTMLSelectElement>('[data-pc-field]').forEach(el => {
...
export function pcTasksBody(tasks: AnyRecord[]): string {
  const rows = tasks.length ? tasks.map(t => {
    const assignee = t.assigneeId ? dbGetById('people', String(t.assigneeId)) as AnyRecord | null : null;
  ...
  }).join('') : `<p style="color:var(--text-tertiary);font-size:.8rem;padding:.5rem 0">No tasks yet.</p>`;
  ...
  document.getElementById('pc-add-task')?.addEventListener('click', () => {
    ...
    const unwatchModal = subscribe(state => {
      if (!state.recordModal) {
        ...
        reloadData();
        setTimeout(() => pcRefreshPanel('tasks'), 50);
      }
    });
  });
...
export function pcBindNotes(_project: AnyRecord): void {
  document.getElementById('pc-add-note')?.addEventListener('click', async () => {
    ...
    const updatedNotes = [...(Array.isArray(existing?.notes) ? (existing!.notes as AnyRecord[]) : []), newNote];
    await dbUpdate('projects', _pcProjectId!, { notes: updatedNotes });
    reloadData(); if (ta) ta.value = '';
    pcRefreshPanel('notes');
    showToast('Note added', 'success');
  });
  document.getElementById('pc-new-note')?.addEventListener('keydown', (e: KeyboardEvent) => {
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) document.getElementById('pc-add-note')?.click();
  });
  document.querySelectorAll<HTMLElement>('[data-del-note]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const existing = dbGetById('projects', _pcProjectId!) as AnyRecord | null;
      const filtered = ((existing?.notes as AnyRecord[] | undefined) || []).filter(n => n.id !== nid);
      await dbUpdate('projects', _pcProjectId!, { notes: filtered });
      reloadData(); pcRefreshPanel('notes');
    });
  });
}
...
export function pcPeopleBody(project: AnyRecord, tasks: AnyRecord[]): string {
  ...
  const team = teamIds.map(id => dbGetById('people', id) as AnyRecord | null).filter(Boolean) as AnyRecord[];
  ...
  }).join('') || `<p style="color:var(--text-tertiary);font-size:.8rem;padding:.25rem 0">No team members yet.</p>`;
...
export function renderProjectCanvas(): void {
  ...
  const project = dbGetById('projects', _pcProjectId) as AnyRecord | null;
  if (!project) { closeProjectCanvas(); return; }
  const saved = (project._canvasLayout || {}) as Record<string, Partial<typeof PC_DEFAULTS[string]>>;
  ...
  overlay.innerHTML = `
    <div class="proj-canvas-topbar">
      ...
      ${tasks.length ? `<div style="display:flex;align-items:center;gap:.5rem;font-size:.8rem;color:var(--text-secondary);flex-shrink:0">
...
export function bindProjectCanvas(): void {
  ...
  document.getElementById('pc-close')?.addEventListener('click', () => {
    pcSaveLayout().then(() => { closeProjectCanvas(); _appRenderWorkspace('projects'); });
  });
  ...
  document.querySelectorAll<HTMLElement>('[data-drag]').forEach(header => {
    ...
    header.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      e.preventDefault();
      _pcTopZ++; panel.style.zIndex = String(_pcTopZ);
    ...
    panel.addEventListener('mousedown', () => { _pcTopZ++; panel.style.zIndex = String(_pcTopZ); });
  });
