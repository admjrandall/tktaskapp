...
function _libSections(docs: AnyRecord[]): string[] {
  ...
  (docs || []).forEach(d => { if (d.section) s.add(String(d.section)); });
  return [...s].sort();
}

function _libTags(docs: AnyRecord[]): string[] {
  const t = new Set<string>();
  (docs || []).forEach(d => ((d.tags as string[] | undefined) || []).forEach(tag => t.add(tag)));
  return [...t].sort();
}
...
function _renderDocCard(d: AnyRecord): string {
  ...
  const sectionChip = d.section ? `<span class="doc-section-chip">${escH(String(d.section))}</span>` : '';
  const tagChips = ((d.tags as string[] | undefined) || []).map(t => `<span class="doc-tag-chip">${escH(t)}</span>`).join('');
...
function _renderFileCard(f: AnyRecord): string {
  const ext = (String(f.name || '')).split('.').pop()?.toUpperCase() ?? 'FILE';
...
export function renderLibrary(state: AppState): string {
  ...
  const filteredDocs = docs.filter(d => {
    ...
    if (q && !String(d.title || '').toLowerCase().includes(q) && !String(d.excerpt || '').toLowerCase().includes(q)) return false;
    return true;
  });
  ...
  const gridHtml = allItems.length ? allItems.join('') : `<div class="lib-empty">No items match your filters.</div>`;
...
export function bindLibrary(): void {
  ...
  document.getElementById('lib-new-doc')?.addEventListener('click', () => openDocumentEditor(null));
  ...
  document.getElementById('lib-file-input')?.addEventListener('change', async e => {
    ...
    for (const file of flist) {
      ...
      await dbCreate('files', { name: file.name, type: file.type, size: file.size, dataUrl, addedAt: nowISO() });
    }
    input.value = '';
    reloadData(); _appRenderWorkspace('library');
...
// ── File viewer modal ─────────────────────────────────────────────────────────
const _TEXT_EXTS_VIEWER = new Set(['txt','md','html','htm','css','js','ts','jsx','tsx','json','xml','csv','yaml','yml','sh','py','rb','go','rs','c','cpp','h','java','sql','log']);
const _IMAGE_EXTS_VIEWER = new Set(['png','jpg','jpeg','gif','webp','svg','bmp','ico']);

function _getFileExtV(name: string): string { return (name || '').split('.').pop()?.toLowerCase() ?? ''; }
function _isTextFileV(name: string): boolean { return _TEXT_EXTS_VIEWER.has(_getFileExtV(name)); }
function _isImageFileV(name: string): boolean { return _IMAGE_EXTS_VIEWER.has(_getFileExtV(name)); }
function _isPdfFileV(name: string): boolean { return _getFileExtV(name) === 'pdf'; }
...
export function bindFileViewer(): void {
  document.getElementById('file-viewer-close')?.addEventListener('click', () => setState({ fileViewer: null }));
