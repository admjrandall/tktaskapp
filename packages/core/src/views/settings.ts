...
import { _idbClearStore } from '../storage/idb-data.js'
import { _vaultDbOpen } from '../security/vault.js'
...
import { isWebAuthnAvailable } from '../security/webauthn.js'
...
let CLOUD_PROVIDERS: Record<
  string,
  { label: string; models: { id: string; label: string; priceIn: number; priceOut: number }[] }
> = {}
...
let _probeOllama: (
  url: string,
) => Promise<{ state: string; version?: string; models?: unknown[] }> = async () => ({
  state: 'down',
})
let _testCloudKey: (
  prov: string,
  key: string,
) => Promise<{ ok: boolean; error?: string }> = async () => ({ ok: false })
...
export function renderSettings(state: AppState): string {
  ...
  const nav = secs
    .map(
      (s) =>
        `<button class="settings-nav-item ${_settingsSection === s.id ? 'active' : ''}" data-section="${s.id}">${s.label}</button>`,
    )
    .join('')
  let body = ''
  ...
  if (_settingsSection === 'general') {
    ...
    const cp = CLOUD_PROVIDERS as Record<
      string,
      { label: string; models: { id: string; label: string; priceIn: number; priceOut: number }[] }
    >
    const tierLabel =
      tier === 'browser'
        ...
        : tier === 'ollama'
          ? isOTOnlyMode()
            ...
            : 'Ollama'
          : tier === 'cloud'
            ? `Cloud · ${cp[(p.cloud as AnyRecord)?.provider as string]?.label || ''}`
            : 'Not configured'
    const modelLabel = (() => {
      if (tier === 'browser')
        return BROWSER_MODELS.find((m) => m.id === (p.browser as AnyRecord)?.modelId)?.label || ''
      if (tier === 'ollama') return String((p.ollama as AnyRecord)?.modelId || '')
      if (tier === 'cloud' && (p.cloud as AnyRecord)?.provider)
        return (
          cp[(p.cloud as AnyRecord).provider as string]?.models.find(
            (m) =>
              m.id ===
              ((p.cloud as AnyRecord).modelByProvider as Record<string, string>)[
                (p.cloud as AnyRecord).provider as string
              ],
          )?.label || ''
        )
      return ''
      ...
      const passkeyList = passkeys.length
        ? passkeys
            .map(
              (
                p,
              ) => `<div style="display:flex;align-items:center;gap:.625rem;padding:.375rem 0;border-bottom:1px solid var(--border-subtle)">
            <div style="flex:1">
    ...
    if (!auditEntries) {
      ...
      const paged = filtered.slice(
        _secAuditPage * AUDIT_PAGE_SIZE,
        (_secAuditPage + 1) * AUDIT_PAGE_SIZE,
      )
      ...
      auditSection = `<div class="card" style="padding:1.25rem;margin-bottom:1rem">
        ...
        ${
          rows
            ? `<div style="overflow-x:auto;margin-bottom:.75rem"><table style="width:100%;border-collapse:collapse">
          <thead><tr>
          ...
          <tbody>${rows}</tbody>
            ...
            : `<p style="font-size:.875rem;color:var(--text-tertiary)">No events match the filter</p>`
        }
        ${
          totalPages > 1
            ? `<div style="display:flex;gap:.5rem;align-items:center;font-size:.8125rem">
    ...
    if (!hasFsApi) {
      ...
      const lastSaveText =
        fsReady && fsLastSave ? `Last saved ${new Date(fsLastSave).toLocaleTimeString()}` : ''
      body = `<h2 style="font-size:1.125rem;font-weight:600;margin-bottom:1.25rem">Storage</h2>
    ...
    const emptyBtn =
      tArr.length > 0
        ? `<button class="btn btn-danger btn-sm" id="recyclebin-empty">Empty Recycle Bin</button>`
        : ''
...
export function bindSettings(state: AppState): void {
  ...
  document.getElementById('reset-app-btn')?.addEventListener('click', () => {
    showConfirm('Delete ALL data and reset?', async () => {
    ...
    try {
      if (file.name.endsWith('.taskappbak')) {
        ...
        if ((result as { legacyKdfUsed?: boolean })?.legacyKdfUsed) {
          setTimeout(() => {
            showToast(
              '⚠️ Backup used older encryption (310k iterations). Export a fresh backup to upgrade it.',
              'info',
              8000,
            )
          }, 1500)
  ...
  document.querySelectorAll<HTMLElement>('.tag-color-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      _tc = (btn.dataset as DOMStringMap & { color: string }).color
      document
        .querySelectorAll<HTMLElement>('.tag-color-btn')
        .forEach((b) => (b.style.outline = ''))
      btn.style.outline = `2px solid ${TAG_COLORS.find((c) => c.label === _tc)?.text || '#000'}`
    })
  })
  ...
  document.getElementById('fs-create-new')?.addEventListener('click', async () => {
    ...
    if (handle) {
      await _fsWriteVault()
      showToast('Vault file created and linked', 'success')
      _appRenderWorkspace('settings')
    } else {
  ...
  document.getElementById('fs-link-existing')?.addEventListener('click', async () => {
    ...
    if (handle) {
      await _fsWriteVault()
      showToast('Vault file linked', 'success')
      _appRenderWorkspace('settings')
    } else {
      showToast('No file selected', 'error')
    }
  ...
  document.getElementById('ai-open-wizard')?.addEventListener('click', () => {
    _openAIWizard((_aiPrefs.tier as string) ? 2 : 1)
  })
  // Nano-only (OT mode) bindings
  document.getElementById('ai-enable-nano')?.addEventListener('click', () => {
    _openAIWizard(1)
  })
  document.getElementById('ai-disable-nano')?.addEventListener('click', async () => {
    _aiPrefs.tier = null
    _aiPrefs.hasCompletedOnboarding = false
    // nanoDisclaimerAcknowledged preserved — user already accepted the one-time warning.
    _saveAIPrefs(_aiPrefs)
    await _resetAIConnection()
    showToast('AI disabled', 'success')
    _appRenderWorkspace('settings')
  })
  if (isOTOnlyMode()) {
    document.getElementById('ai-reconnect')?.addEventListener('click', () => {
      _startAILoad()
    })
  } else {
    document.getElementById('ai-reconnect')?.addEventListener('click', async () => {
      const url = (
        document.getElementById('ai-ollama-url') as HTMLInputElement | null
      )?.value?.trim()
      if (url) (_aiPrefs.ollama as AnyRecord).url = url
      if ((_aiPrefs.tier as string) === 'ollama') {
        try {
          assertLocalAIEndpointAllowed(String((_aiPrefs.ollama as AnyRecord).url || ''))
        } catch (e) {
  ...
  document.querySelectorAll<HTMLInputElement>('input[name="ai-browser-model"]').forEach((el) => {
    el.addEventListener('change', async () => {
      ;(_aiPrefs.browser as AnyRecord).modelId = el.value
      _saveAIPrefs(_aiPrefs)
      await _resetAIConnection()
      _startAILoad()
      showToast(
        `Switched to ${BROWSER_MODELS.find((m) => m.id === el.value)?.label || el.value}`,
        'success',
      )
    })
  })
  document.getElementById('ai-clear-cache')?.addEventListener('click', async () => {
    try {
      if (window.caches) {
        ...
        for (const k of keys) if (/transformers|huggingface/i.test(k)) await caches.delete(k)
      }
      ;(_aiPrefs.browser as AnyRecord).weightsCached = { 'gemma-e2b': false, 'gemma-e4b': false }
      _saveAIPrefs(_aiPrefs)
      showToast('Cached weights cleared (reload page to free memory)', 'success')
    } catch (e) {
  ...
  document.getElementById('ai-ollama-probe')?.addEventListener('click', async () => {
    ...
    if (url) {
      ;(_aiPrefs.ollama as AnyRecord).url = url
      _saveAIPrefs(_aiPrefs)
    }
    try {
      assertLocalAIEndpointAllowed(String((_aiPrefs.ollama as AnyRecord)?.url || ''))
    ...
    const msg =
      r.state === 'ok'
        ? `Reachable (Ollama ${r.version || ''}, ${r.models?.length || 0} models)`
        : r.state === 'cors'
          ? 'CORS-blocked — set OLLAMA_ORIGINS and restart Ollama'
          : r.state === 'down'
            ? 'Not running'
            : 'Bad URL'
    showToast(msg, r.state === 'ok' ? 'success' : 'error', 5000)
  })
  document.getElementById('ai-ollama-browse')?.addEventListener('click', () => {
    _openAIWizard(3)
    _aiWizard.tier = 'ollama'
  ...
  document.getElementById('ai-cloud-model')?.addEventListener('change', (e) => {
    if ((_aiPrefs.cloud as AnyRecord)?.provider) {
      ;((_aiPrefs.cloud as AnyRecord).modelByProvider as Record<string, string>)[
        (_aiPrefs.cloud as AnyRecord).provider as string
      ] = (e.target as HTMLSelectElement).value
      _saveAIPrefs(_aiPrefs)
      showToast('Model updated', 'success')
    }
  })
  document.getElementById('ai-cloud-replace-key')?.addEventListener('click', () => {
    _openAIWizard(3)
    _aiWizard.tier = 'cloud'
    _aiWizard.draft = { cloudProvider: (_aiPrefs.cloud as AnyRecord)?.provider }
    _fullRender(getState())
  })
  document.getElementById('ai-cloud-remove-key')?.addEventListener('click', () => {
    showConfirm('Remove API key for this provider?', async () => {
      const allSecrets = await _aiSecretsLoad()
      const providerKey = String((_aiPrefs.cloud as AnyRecord)?.provider ?? '')
      const { [providerKey]: _removed, ...secrets } = allSecrets
      await _aiSecretsSave(secrets)
      showToast('Key removed', 'success')
      _appRenderWorkspace('settings')
    })
  })
  document.getElementById('ai-cloud-test')?.addEventListener('click', async () => {
    const prov = String((_aiPrefs.cloud as AnyRecord)?.provider || '')
    const k = _getAISecrets()[prov]
    if (!k) {
      showToast('No key set', 'error')
      return
    }
    showToast('Testing…', 'info')
    const r = await _testCloudKey(prov, String(k))
    showToast(
      r.ok ? 'Key valid' : 'Failed: ' + (r.error || 'unknown'),
      r.ok ? 'success' : 'error',
      5000,
    )
  })
  document.getElementById('ai-auto-apply')?.addEventListener('change', (e) => {
    _aiPrefs.autoApplyCreates = (e.target as HTMLInputElement).checked
    _saveAIPrefs(_aiPrefs)
  })
  ...
  document.getElementById('ai-forget-keys')?.addEventListener('click', () => {
    showConfirm(
      'Forget all stored API keys? Cloud AI will stop working until you add them again.',
      async () => {
        await _aiSecretsWipe()
        showToast('All API keys forgotten', 'success')
        _appRenderWorkspace('settings')
      },
    )
  })
  document.getElementById('ai-reset-setup')?.addEventListener('click', () => {
    showConfirm(
      'Reset AI setup? You will be walked through the wizard again. Chat history is preserved.',
      async () => {
        _aiPrefs.hasCompletedOnboarding = false
        _aiPrefs.tier = null
        _saveAIPrefs(_aiPrefs)
        await _resetAIConnection()
        showToast('AI setup reset', 'success')
        _appRenderWorkspace('settings')
      },
    )
  })
  ...
  if (_settingsSection === 'security') {
    ...
    const _maybeRender = () => {
      if (_allLoaded()) _appRenderWorkspace('settings')
    }
    ...
    document.getElementById('totp-setup-verify')?.addEventListener('click', async () => {
      ...
      const { verifyTOTPCode: verifyCb } = await import('../security/totp.js')
